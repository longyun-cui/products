...
<div class="item-piece item-option event-option">

    <!-- BEGIN PORTLET-->
    <div class="boxe panel-default item-entity-container">

        <div class="box-body item-title-row">
            <span><a href="{{url('/event?id='.encode($data->id))}}" target="_blank">{{$data->title or ''}}</a></span>
        </div>

        <div class="box-body item-info-row">
            <span>【{{ $data->category or '未知' }}】</span>
            <span>【{{ $data->start_time or '未知' }} ~ {{ $data->end_time or '未知' }}】</span>
        </div>
        ...
        {{--<div class="box-body item-products-row">--}}
        {{--@foreach($data->products as $pr)--}}
        {{--<span><a href="{{url('/product?id='.encode($pr->id))}}" target="_blank">{{$pr->title or '未知'}}</a></span><br>--}}
        {{--@endforeach--}}
        {{--</div>--}}
        {{--@endif--}}
